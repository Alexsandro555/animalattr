        ...
        <v-layout wrap row>
            <v-flex xs12 class="text-xs-center">
                ...
                <v-card v-if="!loader">
                        ...
                        <v-data-table :headers="headers"
                            ...
                            <template slot="items" slot-scope="props">
                                ...
                                <td class="justify-center layout px-0">
                                    ...
                                    <v-btn icon class="mx-0" @click="$router.push('/Attribute/Edit/'+props.item.id)">
                    ...
                    <v-card-actions>
                        <v-btn @click="add" color="primary" dark class="text-left mb-2">
                            <v-icon>add</v-icon>
                        </v-btn>
                    </v-card-actions>
                </v-card>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapState('Attribute', ['items']),
        },
        methods: {
            addAttribute() {
                this.add()
                    .then(response => {
                        this.$router.push({name: 'Edit-Attribute', params: { id: response.id.toString()}})
                    })
                    .catch(error => {})
            },
            goToPage(item) {
                let url = '/Attribute/detail/'
